...
import EqDeletePopUp from '../../components/EquipmentManagement/EqDeletePopUp'
...
const EqList = (): JSX.Element => {
  ...
  useEffect(() => {
    axios
      .get('http://localhost:8000/eqList/deletedList')
  ...
  for (let i = 0; i < eqList.length; i++) {
    data.push({
      ...
      Delete: <EqDeletePopUp equipmentListId={eqList[i]?._id} />
    })
  }
  ...
  const reportDownload = async () => {
    try {
      ...
      const worksheet = workbook.addWorksheet('Deleted_Equipment')
      ...
      anchor.download = 'Deleted_Equipment_Lists.xlsx'
      anchor.click()
  ...
  return (
    <>
      ...
      <Button
        ...
        style={{marginLeft: '91%', maxWidth: '100%'}}
      >
        Report
      </Button>
      <br />
